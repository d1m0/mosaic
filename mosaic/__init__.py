from flask import Flask
from flask_sqlalchemy import SQLAlchemy
from config import basedir, ADMINS, MAIL_SERVER, MAIL_PORT, MAIL_USERNAME, MAIL_PASSWORD, MAX_VIDEO_SIZE
from flask_uploads import UploadSet, configure_uploads, patch_request_class
import os

app = Flask(__name__)
app.config.from_object('config')

if not app.debug:
    import logging
    from logging.handlers import SMTPHandler, RotatingFileHandler

    # TODO: Email on error
    #credentials = None
    #if MAIL_USERNAME or MAIL_PASSWORD:
    #    credentials = (MAIL_USERNAME, MAIL_PASSWORD)
    #mail_handler = SMTPHandler((MAIL_SERVER, MAIL_PORT), 'no-reply@' + MAIL_SERVER, ADMINS, 'mosaic failure', credentials)
    #mail_handler.setLevel(logging.ERROR)
    #app.logger.addHandler(mail_handler)

    err_file_handler = RotatingFileHandler('tmp/mosaic.err', 'a', 1 * 1024 * 1024, 10)
    err_file_handler.setFormatter(logging.Formatter('%(asctime)s %(levelname)s: %(message)s [in %(pathname)s:%(lineno)d]'))
    err_file_handler.setLevel(logging.ERROR)
    app.logger.addHandler(err_file_handler)

    # Log all info to tmp/mosaic.log
    file_handler = RotatingFileHandler('tmp/mosaic.log', 'a', 1 * 1024 * 1024, 10)
    file_handler.setFormatter(logging.Formatter('%(asctime)s %(levelname)s: %(message)s [in %(pathname)s:%(lineno)d]'))
    app.logger.setLevel(logging.INFO)
    file_handler.setLevel(logging.INFO)

    app.logger.info('mosaic startup')

db = SQLAlchemy(app)


videoSet = UploadSet('videos', ['mov', 'mp4', 'avi', 'mpg', 'mpeg', 'webm', 'm4v', 'wmv', 'flv'])
configure_uploads(app, videoSet)
patch_request_class(app, MAX_VIDEO_SIZE)

from mosaic import views, models
